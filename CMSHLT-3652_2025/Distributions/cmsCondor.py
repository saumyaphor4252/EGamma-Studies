...
# Load the GRun menu
from HLTrigger.Configuration.HLT_2025E_Distributions_cff import *
...
#process.hltEcalRecHitSerialSync.ChannelStatusToBeExcluded = ['kDAC', 'kNoisy', 'kNNoisy', 'kFixedG6', 'kFixedG1', 'kFixedG0', 'kNonRespondingIsolated', 'kDeadVFE', 'kDeadFE', 'kNoDataNoTP']

# Modify the Global Tag
...
process.hltOutputMinimal.fileName = "hlt_Output.root"
...
for i in range(0, nJobs):
    ...
    tmp_job.write("cmsRun hlt_ConfigDump_Reference.py\n")
    tmp_job.write("echo 'sending the file back'\n")
    tmp_job.write("cp hlt_Output.root %s/hlt_Output_%s.root\n"%(remoteDir, str(i)))
    tmp_job.write("rm hlt_Output.root\n")
    tmp_job.close()
    ...
    tmp_cfgFile = open(jobDir+'/hlt_ConfigDump_Reference.py','w')
